...
class EpipolarBasedLoss(BaseLoss, CorrrespondenceUtils):
    """Epipolar Loss. This loss is used for training NeRF with epipolar constraint."""
    ...
    def __init__(self, opt: Dict[str, Any], nerf_net: torch.nn.Module, flow_net: torch.nn.Module,
        ...
        self.grid_flat = self.grid[:, :, 1] * W + self.grid[:, :,
                                                  0]  # (H, W), corresponds to index in flattedned array (in H*W)
    ...
    def compute_loss_pairwise(self, opt: Dict[str, Any], data_dict: Dict[str, Any],
        ...
        stats_dict, plotting_dict, loss_dict = {}, {}, {'corres': torch.tensor(0., requires_grad=True).to(self.device),
                                                        'render_matches': torch.tensor(0., requires_grad=True).to(
                                                            self.device),
                                                        'epipolar': torch.tensor(0., requires_grad=True).to(
                                                            self.device)
                                                        }
        ...
        return self.compute_loss_at_given_img_indexes(opt, data_dict, id_self, id_matching_view,
                                                      corres_map_self_to_other_,
                                                      conf_map_self_to_other_, variance_self_to_other_,
                                                      mask_correct_corr,
    ...
    def compute_loss_at_given_img_indexes(self, opt: Dict[str, Any], data_dict: Dict[str, Any], id_self: int,
        ...
        corres_map_self_to_other_rounded_flat = \
            corres_map_self_to_other_rounded[:, :, 1] * W + corres_map_self_to_other_rounded[:, :,
                                                            0]  # corresponds to index in flattedned array (in H*W)
        # (H, W)
        ...
        with torch.no_grad():
            ...
            if (not skip_verif or self.opt.use_gt_correspondences) \
                ...
                corres_map_and_mask_self_to_other_gt = self.gt_corres_map_and_mask_all_to_all[
                    id_self, id_matching_view]  # (3, H, W)
                ...
                error = torch.norm(corres_map_self_to_other.float() - corres_map_self_to_other_gt.float(), dim=-1,
                                   keepdim=True)
                ...
                stats_dict['pck_1_in_conf'] = error_conf.le(1.).float().mean() if len(
                    error_conf) > 0 else torch.as_tensor(0.)
                stats_dict['pck_3_in_conf'] = error_conf.le(3.).float().mean() if len(
                    error_conf) > 0 else torch.as_tensor(0.)
                ...
                if self.opt.use_gt_correspondences:
                    ...
                    corres_map_self_to_other_rounded_flat = corres_map_self_to_other_rounded[:, :,
                                                            1] * W + corres_map_self_to_other_rounded[:, :,
                                                                     0]  # corresponds to index in flattedned array (in H*W)
        ...
        return self.compute_loss_on_image_pair(data_dict, images, poses_w2c, intrs, id_self, id_matching_view,
                                               ...
                                               conf_map_self_to_other, mask_correct_corr, pose_w2c_self, pose_w2c_other,
                                               intr_self,
