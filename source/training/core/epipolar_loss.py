...
class CorrespondencesPairRenderDepthAndGet3DPtsAndReprojectAndEpipolar(EpipolarBasedLoss):
    ...
    def compute_bearing_vectors(self, opt, pixel_coords, K, R): 
        """
        ...
        """
        points = torch.cat([pixel_coords, torch.ones(len(pixel_coords), 1, device='cuda:0')], dim=-1)
    ...
    def energy_function_without_uncertainty(self, opt, pose_w2c_self, pose_w2c_other, bearing_pixels_in_self, bearing_pixels_in_other, confidences):
        ...
        t_norms = torch.norm(t_rel, dim=-1)
        t_rel = t_rel / t_norms[..., None]
        ...
        transformed_bearing_pixels_in_other = torch.einsum("...ij,...nj->...ni", R_rel, bearing_pixels_in_other) 
